...
from verl.utils.device import get_device_name, get_torch_device, is_cuda_available, is_npu_available, get_nccl_backend, get_device_id
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            if vllm_mode == "customized":
                ...
                print(f"model_path: {local_path}, config: {self.config.rollout}, tokenizer: {self.tokenizer}, model_hf_config: {self.actor_model_config}, trust_remote_code: {trust_remote_code}, lora_kwargs: {lora_kwargs}")
...
class QEWorker(Worker):
    ...
    def init_model(self):
        # This is used to import external_lib into the huggingface systems
        import_external_libs(self.config.model.get('external_lib', None))
        from verl.workers.qe import DataParallelQE
        ...
        self.comet = DataParallelQE(config=self.config, comet_module=self.comet_module, tokenizer = self.tokenizer)

        torch.cuda.empty_cache()

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_qe_rm(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            comet_scores = self.comet.compute_qe_rm(data=data)
            output = DataProto.from_dict(tensors={'qe_rm': comet_scores})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
        output = output.to('cpu')
    ...
    def compute_valid_qe(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            comet_scores = self.comet.compute_valid_qe(data=data)
            output = DataProto.from_dict(tensors={'valid_qe_metric': comet_scores})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
        output = output.to('cpu')
