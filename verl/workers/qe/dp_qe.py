...
from verl.workers.qe import BaseQEModel
...
__all__ = ['DataParallelQE']


class DataParallelQE(BaseQEModel):

    def __init__(self, config, comet_module, tokenizer):
        super().__init__(config=config)
        self.comet_model = comet_module
        print(f"mini_batch_size: {self.config.ppo_mini_batch_size}, micro_batch_size: {self.config.ppo_micro_batch_size}")
    ...
    def extract_translation(self, solution_str: str):
        ...
        src, tgt = self.config.src_lang, self.config.tgt_lang
        base_template = f"Translate from {src} to {tgt}:"
        if base_template in solution_str: # base
            processed_str = solution_str.split(base_template, 1)[1]
    ...
    def compute_qe_rm(self, data: DataProto) -> torch.Tensor:
    ...
    def compute_valid_qe(self, data: DataProto) -> torch.Tensor:
