...
from verl.utils.reward_score.mt_score import compute_spBLEU

def default_compute_score(data_source, prompt_str, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None):
    ...
    elif data_source in ["qe"]:
        from . import qe
        res = qe.compute_score.remote(prompt_str, solution_str, ground_truth)
        res = ray.get(res)
    else:
...
__all__ = ["default_compute_score", "compute_spBLEU"]
