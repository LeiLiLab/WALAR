...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        if self.config.qe_model.enable:
            resource_pool = self.resource_pool_manager.get_resource_pool(Role.QEModel)
            qe_cls = RayClassWithInitArgs(self.role_worker_mapping[Role.QEModel], config=self.config.qe_model)
            self.resource_pool_to_cls[resource_pool]["qe"] = qe_cls
        ...
        if self.config.qe_model.enable:
            self.qe_wg = all_wg["qe"]
            self.qe_wg.init_model()
        else:
            self.qe_wg = None
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if self.qe_model.enable:
                # compute comet rm
                qe_result = self.qe_wg.compute_valid_qe(test_batch)
                test_batch = test_batch.union(qe_result)
                qe_reward_tensor = qe_result["valid_qe_metric"].batch
                qe_tensor_lst.append(qe_reward_tensor)
            ...
            bleu_score_tensor = self.val_reward_fn(test_data)
            bleu_tensor_lst.append(bleu_score_tensor)
        ...
        if len(bleu_tensor_lst) > 0:
            bleu_tensor = torch.cat(bleu_tensor_lst, dim=0).sum(-1).cpu()  # (batch_size,)
            avg_bleu = bleu_tensor.mean().item() if bleu_tensor.numel() > 0 else 0
            metric_dict['bleu'] = avg_bleu
        # Calculate average COMET score if available
        if len(qe_tensor_lst) > 0:
            comet_scores = torch.cat(qe_tensor_lst, dim=0).cpu().tolist()
            avg_comet = sum(comet_scores) / len(comet_scores) if comet_scores else 0
            metric_dict["comet"] = avg_comet
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.qe_wg is not None:
                            with _timer('qe_rm', timing_raw):
                                qe_reward = self.qe_wg.compute_qe_rm(batch)
                                batch = batch.union(qe_reward)
                        
                        # collate the reward into tensor
                    ...
                    if rollout_data_dir:
                        with _timer("dump_rollout_generations", timing_raw):
                            ...
                            inputs = self.tokenizer.batch_decode(batch.batch["prompts"])
